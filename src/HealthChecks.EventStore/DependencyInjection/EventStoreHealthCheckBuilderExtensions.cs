...
namespace Microsoft.Extensions.DependencyInjection
{
    public static class EventStoreHealthCheckBuilderExtensions
    {
        ...
        /// <param name="builder">The <see cref="IHealthChecksBuilder"/>.</param>
        /// <param name="eventStoreConnection">The EventStore connection string to be used.</param>
        /// <param name="eventStoreConnectionSettings">The EventStore Connection Settings Builder used to establish the connection.</param>
        /// <param name="name">The health check name. Optional. If <c>null</c> the type name 'eventstore' will be used for the name.</param>
        /// <param name="failureStatus">
        /// The <see cref="HealthStatus"/> that should be reported when the health check fails. Optional. If <c>null</c> then
        /// the default status of <see cref="HealthStatus.Unhealthy"/> will be reported.
        ...
        /// <param name="timeout">An optional System.TimeSpan representing the timeout of the check.</param>
        /// <returns>The <see cref="IHealthChecksBuilder"/>.</returns>
        public static IHealthChecksBuilder AddEventStore(this IHealthChecksBuilder builder, string eventStoreConnection, EventStore.ClientAPI.ConnectionSettingsBuilder eventstoreConnectionSettings, string name = default, HealthStatus? failureStatus = default, IEnumerable<string> tags = default, TimeSpan? timeout = default)
        {
            return builder.Add(new HealthCheckRegistration(
                name ?? NAME,
                sp => new EventStoreHealthCheck(eventStoreConnection, eventstoreConnectionSettings),
                failureStatus,
                tags,
                timeout));
        }
